#include <queue>
#include <set>
#include "BfsSolver.h"

using namespace PPSolver;

Match BfsSolver::_makeMatch(Coords c1, Coords c2)
{
  Match m;
  if (c1 < c2) {
    m.first = c1;
    m.second = c2;
  } else {
    m.first = c2;
    m.second = c1;
  }
  return m;
}

std::vector<Match> BfsSolver::solve()
{
  std::set<Match> found;
  std::vector<Match> result;
  std::queue<BfsSolver::Vertex> q;
  // v = current vertex, n = new vertex
  BfsSolver::Vertex v, n;
  Coords c, nc, start;
  for (unsigned i = 0; i < ICellMatrix::CELLS_PER_ROW; i++) {
    for (unsigned j = 0; j < ICellMatrix::CELLS_PER_COL; j++) {
      // Construct starting vertex
      start.x = i;
      start.y = j;
      v.coords.x = i;
      v.coords.y = j;
      v.turns = 3; // @todo define as constant value
      v.direction = BfsSolver::Vertex::None;
      q.push(v);
      while (!q.empty()) {
        v = q.front();
        q.pop();
        c = v.coords;

        // Go up
        if (
          v.direction != BfsSolver::Vertex::Down
          && c.y > 0
          && (v.direction == BfsSolver::Vertex::Up || v.turns > 0)
        ) {
          nc.x = c.x;
          nc.y = c.y - 1;
          if (this->matrix.isEmpty(nc)) {
            // Make new vertex
            n.turns = v.turns;
            if (v.direction != BfsSolver::Vertex::Up) {
              n.turns--;
            }
            n.direction = BfsSolver::Vertex::Up;
            n.coords = nc;
            q.push(n);
          } else {
            // Check whether blocking cell is the same
            if (this->matrix.isEqual(start, nc)) {
              Match m = this->_makeMatch(start, nc);
              if (found.count(m) == 0) {
                found.insert(m);
                result.push_back(m);
              }
            }
          }
        }

        // Go right
        if (
          v.direction != BfsSolver::Vertex::Left
          && c.x < ICellMatrix::CELLS_PER_ROW - 1
          && (v.direction == BfsSolver::Vertex::Right || v.turns > 0)
        ) {
          nc.x = c.x + 1;
          nc.y = c.y;
          if (this->matrix.isEmpty(nc)) {
            // Make new vertex
            n.turns = v.turns;
            if (v.direction != BfsSolver::Vertex::Right) {
              n.turns--;
            }
            n.direction = BfsSolver::Vertex::Right;
            n.coords = nc;
            q.push(n);
          } else {
            // Check whether blocking cell is the same
            if (this->matrix.isEqual(start, nc)) {
              Match m = this->_makeMatch(start, nc);
              if (found.count(m) == 0) {
                found.insert(m);
                result.push_back(m);
              }
            }
          }
        }
      }
    }
  }
  return result;
}

